...
def printarg( X ):
  print("Długość ciągu X: ", limit(len(X),120))
  print("Ciąg X         : ", limit(X,120))

def printhint( hint ):
  print("Długość najdłuższego podciągu MR:", limit(hint,120) )
  
def printsol( sol ):
  print("Długość uzyskanego ciągu        :" , len(sol) )
  print("Uzyskany ciąg                   :" , limit(sol,120) )
...
def check( X, hint, sol ):
    ...
    while i < len(sol) and prev < sol[i]:
      ...
      i += 1
    if i != len(sol):
      print("Błąd! Ciąg nie jest typu MR!")
      return False
    ...
    if j < len(sol):
      print("Błąd! Wynik nie jest podciągiem!")
      return False

    if len(sol) != hint:
      print("Błąd! Za krótki podciąg")
      return False
